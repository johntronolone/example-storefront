...
import addTypographyStyles from "components/CartItems/utils/addTypographyStyles";
import applyTheme from "components/CartItems/utils/applyTheme";
import CustomPropTypes from "components/CartItems/utils/CustomPropTypes";
import QuantityInputMod from "./QuantityInputMod.js";
...
class CartItemMod extends Component {
  static propTypes = {
    /**
  ...
  renderImage() {
    const { isMiniCart, item, item: { imageURLs, productSlug }, productURLPath } = this.props;

    const { small, thumbnail } = imageURLs || {};
    
    //console.log({item});
    //console.log({productURLPath});
    ...
    return (
      <a style={{display: 'block'}}  href={[productURLPath, productSlug].join("")}>
        <picture>
          {/*<source srcSet={"https://backend.patchamp.com"+small} media="(min-width: 768px)" />*/}
          <img src={"https://backend.patchamp.com"+thumbnail} alt="" style={{ display: "block" }} />
        </picture>
      </a>
    );
  }
            ...
            <ItemContentDetailInner>
              ...
              {!isReadOnly &&
                <ItemContentQuantityInput isMiniCart={isMiniCart}>
                  <QuantityInputMod value={quantity} onChange={this.handleChangeCartItemQuantity} />
...
export default withComponents(CartItemMod);
